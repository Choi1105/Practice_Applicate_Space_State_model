%% State-Space Model
% Unobserved Component Model - Practice Version
% Y(t) = N(t) + X(t), 
% N(t) = Mu + N(t-1) + u(t), u(t) ~ iid(0,sig2u)
% X(t) = phi1*X(t-1) + phi2*X(t-2) + e(t), e(t) ~ iidN (0, sig2e)

% Y(t) = Real GDP
% N(t) = Trend
% X(t) = Cyclical Component
% 'Real GDP' can be decompose 'Trend' and 'Cyclical Component'

% Measurement equation
% y(t) = H*B(t) 

% Transition equation
% B(t) = Mu + F*B(t-1) + v(t) ~ N(0,Q)
...
nm(1) = mu + randn(1,1)*sqrt(sig2u);
nm(2) = mu + nm(1) + randn(1,1)*sqrt(sig2u);

xm(1) = randn(1,1)*sqrt(sig2v);
xm(2) = phi1*xm(1) + randn(1,1)*sqrt(sig2v);
xm(3) = phi1*xm(2) + phi2*xm(1) + randn(1,1)*sqrt(sig2v);

% Generate n(t) for t = 3 ~ T
for t = 3:T
    
    nm(t) = mu + nm(t-1) + randn(1,1)*sqrt(sig2u);
   
    
end
% Generate x(t) for t = 3 ~ T
for t = 3:T
    
    xm(t) = phi1*xm(t-1) + phi2*xm(t-2) + randn(1,1)*sqrt(sig2v);
...
figure
plot(i, nm ,'k', i, Beta_ttm(:,2), 'r:', 'LineWidth',1.5);
legend('True','Filtered'); 
