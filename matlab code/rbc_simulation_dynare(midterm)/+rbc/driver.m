%
% Status : main Dynare file
%
% Warning : this file is generated automatically by Dynare
%           from model file (.mod)

if isoctave || matlab_ver_less_than('8.6')
    clear all
else
    clearvars -global
    clear_persistent_variables(fileparts(which('dynare')), false)
end
tic0 = tic;
% Define global variables.
global M_ options_ oo_ estim_params_ bayestopt_ dataset_ dataset_info estimation_info ys0_ ex0_
options_ = [];
M_.fname = 'rbc';
M_.dynare_version = '5.1';
oo_.dynare_version = '5.1';
options_.dynare_version = '5.1';
%
% Some global variables initialization
%
global_initialization;
M_.exo_names = cell(2,1);
M_.exo_names_tex = cell(2,1);
M_.exo_names_long = cell(2,1);
M_.exo_names(1) = {'eps_a'};
M_.exo_names_tex(1) = {'eps\_a'};
M_.exo_names_long(1) = {'eps_a'};
M_.exo_names(2) = {'eps_v'};
M_.exo_names_tex(2) = {'eps\_v'};
M_.exo_names_long(2) = {'eps_v'};
M_.endo_names = cell(12,1);
M_.endo_names_tex = cell(12,1);
M_.endo_names_long = cell(12,1);
M_.endo_names(1) = {'c'};
M_.endo_names_tex(1) = {'c'};
M_.endo_names_long(1) = {'c'};
M_.endo_names(2) = {'n'};
M_.endo_names_tex(2) = {'n'};
M_.endo_names_long(2) = {'n'};
M_.endo_names(3) = {'k'};
M_.endo_names_tex(3) = {'k'};
M_.endo_names_long(3) = {'k'};
M_.endo_names(4) = {'w'};
M_.endo_names_tex(4) = {'w'};
M_.endo_names_long(4) = {'w'};
M_.endo_names(5) = {'R'};
M_.endo_names_tex(5) = {'R'};
M_.endo_names_long(5) = {'R'};
M_.endo_names(6) = {'y'};
M_.endo_names_tex(6) = {'y'};
M_.endo_names_long(6) = {'y'};
M_.endo_names(7) = {'i'};
M_.endo_names_tex(7) = {'i'};
M_.endo_names_long(7) = {'i'};
M_.endo_names(8) = {'delta_u'};
M_.endo_names_tex(8) = {'delta\_u'};
M_.endo_names_long(8) = {'delta_u'};
M_.endo_names(9) = {'u'};
M_.endo_names_tex(9) = {'u'};
M_.endo_names_long(9) = {'u'};
M_.endo_names(10) = {'r'};
M_.endo_names_tex(10) = {'r'};
M_.endo_names_long(10) = {'r'};
M_.endo_names(11) = {'v'};
M_.endo_names_tex(11) = {'v'};
M_.endo_names_long(11) = {'v'};
M_.endo_names(12) = {'a'};
M_.endo_names_tex(12) = {'a'};
M_.endo_names_long(12) = {'a'};
M_.endo_partitions = struct();
M_.param_names = cell(12,1);
M_.param_names_tex = cell(12,1);
M_.param_names_long = cell(12,1);
M_.param_names(1) = {'beta'};
M_.param_names_tex(1) = {'beta'};
M_.param_names_long(1) = {'beta'};
M_.param_names(2) = {'delta'};
M_.param_names_tex(2) = {'delta'};
M_.param_names_long(2) = {'delta'};
M_.param_names(3) = {'alpha'};
M_.param_names_tex(3) = {'alpha'};
M_.param_names_long(3) = {'alpha'};
M_.param_names(4) = {'chi'};
M_.param_names_tex(4) = {'chi'};
M_.param_names_long(4) = {'chi'};
M_.param_names(5) = {'rho_a'};
M_.param_names_tex(5) = {'rho\_a'};
M_.param_names_long(5) = {'rho_a'};
M_.param_names(6) = {'rho_v'};
M_.param_names_tex(6) = {'rho\_v'};
M_.param_names_long(6) = {'rho_v'};
M_.param_names(7) = {'sigma_a'};
M_.param_names_tex(7) = {'sigma\_a'};
M_.param_names_long(7) = {'sigma_a'};
M_.param_names(8) = {'sigma_v'};
M_.param_names_tex(8) = {'sigma\_v'};
M_.param_names_long(8) = {'sigma_v'};
M_.param_names(9) = {'phi_1'};
M_.param_names_tex(9) = {'phi\_1'};
M_.param_names_long(9) = {'phi_1'};
M_.param_names(10) = {'phi_2'};
M_.param_names_tex(10) = {'phi\_2'};
M_.param_names_long(10) = {'phi_2'};
M_.param_names(11) = {'k_n'};
M_.param_names_tex(11) = {'k\_n'};
M_.param_names_long(11) = {'k_n'};
M_.param_names(12) = {'theta'};
M_.param_names_tex(12) = {'theta'};
M_.param_names_long(12) = {'theta'};
M_.param_partitions = struct();
M_.exo_det_nbr = 0;
M_.exo_nbr = 2;
M_.endo_nbr = 12;
M_.param_nbr = 12;
M_.orig_endo_nbr = 12;
M_.aux_vars = [];
M_ = setup_solvers(M_);
M_.Sigma_e = zeros(2, 2);
M_.Correlation_matrix = eye(2, 2);
M_.H = 0;
M_.Correlation_matrix_ME = 1;
M_.sigma_e_is_diagonal = true;
M_.det_shocks = [];
M_.surprise_shocks = [];
M_.heteroskedastic_shocks.Qvalue_orig = [];
M_.heteroskedastic_shocks.Qscale_orig = [];
options_.linear = false;
options_.block = false;
options_.bytecode = false;
options_.use_dll = false;
M_.orig_eq_nbr = 12;
M_.eq_nbr = 12;
M_.ramsey_eq_nbr = 0;
M_.set_auxiliary_variables = exist(['./+' M_.fname '/set_auxiliary_variables.m'], 'file') == 2;
M_.epilogue_names = {};
M_.epilogue_var_list_ = {};
M_.orig_maximum_endo_lag = 1;
M_.orig_maximum_endo_lead = 1;
M_.orig_maximum_exo_lag = 0;
M_.orig_maximum_exo_lead = 0;
M_.orig_maximum_exo_det_lag = 0;
M_.orig_maximum_exo_det_lead = 0;
M_.orig_maximum_lag = 1;
M_.orig_maximum_lead = 1;
M_.orig_maximum_lag_with_diffs_expanded = 1;
M_.lead_lag_incidence = [
 0 6 18;
 0 7 0;
 1 8 0;
 0 9 0;
 0 10 19;
 0 11 0;
 2 12 0;
 3 13 20;
 0 14 21;
 0 15 0;
 4 16 0;
 5 17 0;]';
M_.nstatic = 4;
M_.nfwrd   = 3;
M_.npred   = 4;
M_.nboth   = 1;
M_.nsfwrd   = 4;
M_.nspred   = 5;
M_.ndynamic   = 8;
M_.dynamic_tmp_nbr = [10; 3; 0; 0; ];
M_.model_local_variables_dynamic_tt_idxs = {
};
M_.equations_tags = {
  1 , 'name' , 'delta_u' ;
  2 , 'name' , '2' ;
  3 , 'name' , '3' ;
  4 , 'name' , 'w' ;
  5 , 'name' , 'R' ;
  6 , 'name' , '6' ;
  7 , 'name' , '7' ;
  8 , 'name' , 'y' ;
  9 , 'name' , '9' ;
  10 , 'name' , 'k' ;
  11 , 'name' , '11' ;
  12 , 'name' , '12' ;
};
M_.mapping.c.eqidx = [2 3 4 9 ];
M_.mapping.n.eqidx = [4 6 7 8 ];
M_.mapping.k.eqidx = [6 7 8 10 ];
M_.mapping.w.eqidx = [4 7 ];
M_.mapping.R.eqidx = [2 5 6 ];
M_.mapping.y.eqidx = [8 9 ];
M_.mapping.i.eqidx = [9 10 ];
M_.mapping.delta_u.eqidx = [1 2 10 ];
M_.mapping.u.eqidx = [1 2 5 6 7 8 ];
M_.mapping.r.eqidx = [3 ];
M_.mapping.v.eqidx = [4 11 ];
M_.mapping.a.eqidx = [6 7 8 12 ];
M_.mapping.eps_a.eqidx = [12 ];
M_.mapping.eps_v.eqidx = [11 ];
M_.static_and_dynamic_models_differ = false;
M_.has_external_function = false;
M_.state_var = [3 7 8 11 12 ];
M_.exo_names_orig_ord = [1:2];
M_.maximum_lag = 1;
M_.maximum_lead = 1;
M_.maximum_endo_lag = 1;
M_.maximum_endo_lead = 1;
oo_.steady_state = zeros(12, 1);
M_.maximum_exo_lag = 0;
M_.maximum_exo_lead = 0;
oo_.exo_steady_state = zeros(2, 1);
M_.params = NaN(12, 1);
M_.endo_trends = struct('deflator', cell(12, 1), 'log_deflator', cell(12, 1), 'growth_factor', cell(12, 1), 'log_growth_factor', cell(12, 1));
M_.NNZDerivatives = [44; -1; -1; ];
M_.static_tmp_nbr = [10; 3; 0; 0; ];
M_.model_local_variables_static_tt_idxs = {
};
M_.params(1) = 0.99;
beta = M_.params(1);
M_.params(2) = 0.02;
delta = M_.params(2);
M_.params(3) = 0.3333333333333333;
alpha = M_.params(3);
M_.params(4) = 1;
chi = M_.params(4);
M_.params(5) = 0.97;
rho_a = M_.params(5);
M_.params(6) = 0.95;
rho_v = M_.params(6);
M_.params(7) = 0.01;
sigma_a = M_.params(7);
M_.params(8) = 0.02;
sigma_v = M_.params(8);
M_.params(9) = 1/M_.params(1)-(1-M_.params(2));
phi_1 = M_.params(9);
M_.params(10) = 0.01;
phi_2 = M_.params(10);
M_.params(11) = ((1/M_.params(1)-(1-M_.params(2)))/M_.params(3))^(1/(M_.params(3)-1));
k_n = M_.params(11);
M_.params(12) = (1-M_.params(3))*M_.params(11)^M_.params(3)/(0.3333333333333333^(1+M_.params(4))*(M_.params(11)^M_.params(3)-M_.params(2)*M_.params(11)));
theta = M_.params(12);
steady;
oo_.dr.eigval = check(M_,options_,oo_);
%
% SHOCKS instructions
%
M_.exo_det_length = 0;
M_.Sigma_e(1, 1) = M_.params(7)^2;
M_.Sigma_e(2, 2) = M_.params(8)^2;
options_.irf = 20;
options_.loglinear = true;
options_.order = 1;
var_list_ = {'y';'n';'c';'i';'u';'w';'r'};
[info, oo_, options_, M_] = stoch_simul(M_, options_, oo_, var_list_);


oo_.time = toc(tic0);
disp(['Total computing time : ' dynsec2hms(oo_.time) ]);
if ~exist([M_.dname filesep 'Output'],'dir')
    mkdir(M_.dname,'Output');
end
save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'oo_', 'M_', 'options_');
if exist('estim_params_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'estim_params_', '-append');
end
if exist('bayestopt_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'bayestopt_', '-append');
end
if exist('dataset_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'dataset_', '-append');
end
if exist('estimation_info', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'estimation_info', '-append');
end
if exist('dataset_info', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'dataset_info', '-append');
end
if exist('oo_recursive_', 'var') == 1
  save([M_.dname filesep 'Output' filesep 'rbc_results.mat'], 'oo_recursive_', '-append');
end
if ~isempty(lastwarn)
  disp('Note: warning(s) encountered in MATLAB/Octave code')
end
