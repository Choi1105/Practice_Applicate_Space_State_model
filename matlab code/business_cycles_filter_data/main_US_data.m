...
US_Y = readmatrix('KR_US_data.xlsx','sheet','데이터','range','J5:J188');
US_I = readmatrix('KR_US_data.xlsx','sheet','데이터','range','K5:K188');
US_C = readmatrix('KR_US_data.xlsx','sheet','데이터','range','L5:L188');
US_TB_Y = readmatrix('KR_US_data.xlsx','sheet','데이터','range','M5:M188');


% HP filter data
chp=US_C-trend(US_C,1600); 
ihp=US_I-trend(US_I,1600);
yhp=US_Y-trend(US_Y,1600); 

% US_data save
datahp_us = [yhp chp ihp US_TB_Y];
save('datahp_us','datahp_us');
...
% 1st difference
c1std=vertcat(0, diff(US_C));
i1std=vertcat(0, diff(US_I));
y1std=vertcat(0, diff(US_Y));

% Band-Pass Filter the data for comparison
pl=6;
pu=32; %This setting isolate the components with periods between 1.5 and 8 yrs
cbp = bpass(US_C,pl,pu);
ibp = bpass(US_I,pl,pu);
ybp = bpass(US_Y,pl,pu);
