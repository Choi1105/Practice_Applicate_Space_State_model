...
la = (c - THETA/OMEGA * h^OMEGA)^(-GAMA);     % Marginal Utility

lap = (cp - THETA/OMEGA * hp^OMEGA)^(-GAMA);  % Marginal Utility
...
e2 = -yy + a * k^ALFA *  (g*h)^(1-ALFA);    % 생산함수

e3 = -invs + kp*g - (1-DELTA) *k;           % 투자함수

e4 = - la  + BETTA / g^GAMA * r * lap;      % 오일러 Equation

e5 = -THETA* h^(OMEGA-1) + (1-ALFA) * a * g^(1-ALFA) * (k/h)^ALFA;     % Labor supply
% e5 = - h^(OMEGA-1) + (1-ALFA) * a * g^(1-ALFA) * (k/h)^ALFA;

e6 = -la * (1+ PHI * ac) + BETTA/ g^GAMA * lap * (1 - DELTA + ALFA * ap * (gp*hp/kp)^(1-ALFA) + PHI * k1p/k1*gp * acp - PHI/2 * acp^2 );   % Capital 

e7 = -k1 + kp;    % k(t+2)의 보조변수

e8 = -log(ap) + RHOA * log(a);  

e9 = -log(gp/G) + RHOG * log(g/G); 

e10 = -tby + tb / yy;  % TB,GDP ratio

e11 = -gy + yy/yyback*gback;   % Output growth ratio

e12 = -gcc + c/cback*gback;    % Consumption growth ratio

e13 = -giv + invs/ivback*gback;  % Investment growth ratio
...
logvar = [gy gcc giv h yy c invs k1 yyback cback ivback gback k d g a];   % tby는 log씌울 수 없음 +가 좋은지 -가 좋은지 모름
