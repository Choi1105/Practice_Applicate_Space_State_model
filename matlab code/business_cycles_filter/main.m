...
%*********************** load data *****************************
% load NIPA data from 1948:1 to 2014:4
load c;
load i;
load y;

i=i(5:end);
% log y,c,i  n is already in the correct units
logc=log(c);
logi=log(i);
logy=log(y);

% HP filter data
chp=logc-trend(logc,1600); 
ihp=logi-trend(logi,1600);
yhp=logy-trend(logy,1600); 
...
% 1st difference
c1std=vertcat(0, diff(logc));
i1std=vertcat(0, diff(logi));
y1std=vertcat(0, diff(logy));

% Band-Pass Filter the data for comparison
pl=6;
pu=32; %This setting isolate the components with periods between 1.5 and 8 yrs
cbp = bpass(logc,pl,pu);
ibp = bpass(logi,pl,pu);
ybp = bpass(logy,pl,pu);
